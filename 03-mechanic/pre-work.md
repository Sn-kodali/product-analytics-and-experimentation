...
Real, not manufactured — uncategorized expenses pile up, invoices age, actual financial risk.
...
**Value churn** — session-duration and financial-modeling usage both drop after the initial setup, meaning users get their one Aha moment and don't return regularly.

## Defend the mechanic

Yes — it gives a user a specific, recurring reason to come back weekly ("3 new uncategorized expenses this week") instead of a vague dashboard-check nudge, which is exactly the gap causing churn.

**Risk to watch:** if clearing items starts to feel like nagging busywork rather than genuinely useful upkeep, it tips into the "reward loop without value" anti-pattern — so it only works as long as categorizing is itself useful to the user, not just a number going down.
